...
<div class="container py-4">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            ...
            <div class="resource-detail-card"><h5>📋 Détails techniques</h5><hr><p><i class="ti-file"></i> Format: PDF</p><p><i class="ti-book"></i> Pages: <?= $resource['pages'] ?> pages</p><p><i class="ti-download"></i> Téléchargements: <?= number_format($resource['downloads']) ?></p><p><i class="ti-star"></i> Note: <?= number_format($resource_rating,1) ?>/5</p><hr><p class="mb-2"><strong>Quiz certifiant</strong></p><p class="text-muted mb-3">10 questions, note sur 10, certificat à partir de 5/10.</p><?php if($currentUser): ?><a href="index.php?action=quiz&subaction=start&id=<?= (int)$resource['id_res'] ?>" class="btn-primary-custom w-100">Lancer le quiz</a><?php else: ?><a href="index.php?action=login" class="btn-outline-custom w-100">Se connecter pour passer le quiz</a><?php endif; ?></div>
        </div>
    </div>
</div>
