...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            ...
            <div class="table-responsive"><table class="table table-hover"><thead class="table-light"><tr><th>ID</th><th>Nom complet</th><th>Email</th><th>Université</th><th>Filière</th><th>Abonnement</th><th>Rôle</th><th>Actions</th></tr></thead><tbody>
                <?php foreach($users as $user): ?>
                <tr>
                    <td><?= $user['id'] ?></td>
                    <td><i class="bi bi-person-circle me-2" style="color:#1a8cff"></i><?= escape($user['prenom'] . ' ' . $user['nom']) ?></td>
                    <td><?= escape($user['email']) ?></td>
                    <td><?= escape($user['universite'] ?: '-') ?></td>
                    <td><?= escape($user['filiere'] ?: '-') ?></td>
                    <td>
                        <?php if (!empty($user['subscription_nom'])): ?>
                            <span class="badge bg-primary"><?= escape($user['subscription_nom']) ?></span>
                            <br><small class="text-muted">Jusqu'au <?= escape($user['subscription_fin']) ?></small>
                        <?php else: ?>
                            <span class="badge bg-light text-dark border">Aucun</span>
                        <?php endif; ?>
                    </td>
                    <td><span class="badge bg-secondary"><?= $user['role'] == 0 ? 'Admin' : 'User' ?></span></td>
                    <td><a href="index.php?action=admin&subaction=edit_user&id=<?= $user['id'] ?>" class="btn-edit"><i class="bi bi-pencil-fill"></i></a><a href="index.php?action=admin&subaction=delete_user&id=<?= $user['id'] ?>" class="btn-delete" onclick="return confirm('Supprimer ?')"><i class="bi bi-trash3-fill"></i></a></td>
                </tr>
                <?php endforeach; ?>
            </tbody></table></div>
        </div>
